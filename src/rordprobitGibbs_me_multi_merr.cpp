...
List rordprobitGibbs_me_multi_merr_cpp_loop(arma::mat const& y, arma::mat const& X, int k, arma::mat const& A, arma::vec const& betabar, arma::mat const& Ad,
  ...
  arma::mat ucholinv = solve(arma::trimatu(chol(trans(X)*X+A)), eye(nvar,nvar)); //trimatu interprets the matrix as upper triangular and makes solve more efficient
  ...
  ucholinv = solve(arma::trimatu(chol(Ad)), eye(ndstar,ndstar));
  ...
  for (int rep=0; rep<R; rep++){
    ...
    for(int ind=0; ind<Y_ind; ind++){
      ...
      //draw gamma
      metropout = dstarRwMetrop(y(span::all,ind),beta_tilde(ind,0)+z,trans(olddstar(ind,span::all)),s,inc_root,dstarbar,oldll[ind],rootdi, ncut, ssq_y_tilde[ind]);
    ...
    arma::mat ucholinv_tilde = solve(arma::trimatu(chol(trans(q)*q+1)), eye(1,1)); //trimatu interprets the matrix as upper triangular and makes solve more efficient
